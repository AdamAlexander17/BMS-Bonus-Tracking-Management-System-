...
import { AuthProvider, getDefaultRouteForUser, isRmOrJrmOnlyUser, useAuth } from './context/AuthContext';
...
function AppRoutes() {
  const { user } = useAuth();
  const scopedBrokerRoute = getDefaultRouteForUser(user);
  const isRmScoped = isRmOrJrmOnlyUser(user) && user?.id;
  ...
  return (
    <Routes>
      ...
      >
        <Route index element={isRmScoped ? <Navigate to={scopedBrokerRoute} replace /> : <Dashboard />} />
        <Route path="users"   element={<Users />} />
        <Route path="roles"   element={<Roles />} />
        <Route path="brokers"              element={isRmScoped ? <Navigate to={scopedBrokerRoute} replace /> : <Brokers />} />
      ...
      <Route path="*" element={<Navigate to={scopedBrokerRoute} replace />} />
    </Routes>
  );
}
